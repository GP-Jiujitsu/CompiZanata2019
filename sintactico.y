...
void displayTupla(FILE *p ,int filas,symbol symbolTable[]){
    int j;
    for(j=0; j < filas; j++ ){
...
void displayTable(FILE *p,  int filas, symbol symbolTable[], void (*tupla)(FILE *p ,int filas, symbol symbolTable[])){   
...
void CrearSymbolTable(symbol symbolTable[],char * ruta){
    ...
    FILE  *p = fopen(ruta, "w");
    displayTable(p, pos_st, symbolTable, displayTupla);
    //Fin
    fclose(p);
}
...
%token IF ELSE WHILE DEFVAR ENDDEF DISPLAY GET AVG INLIST
...
declaracion:
    tipo_dato D_P lista_variables     {   fprintf(stdout,"\ndeclaracion - lista_variables D_P tipo_dato");    
                                        fflush(stdout); }
    ;

lista_variables: 
    lista_variables PUNTO_Y_COMA ID {   collectId(yylval.s);
...
escritura:
    DISPLAY expresion PUNTO_Y_COMA        {   fprintf(stdout,"\nescritura - DISPLAY expresion PUNTO_Y_COMA");
                                            fflush(stdout);
                                            apilarPolaca("DISPLAY");
                                            resetTipos();   }
    | DISPLAY concatenacion PUNTO_Y_COMA  {   fprintf(stdout,"\nescritura - DISPLAY concatenacion PUNTO_Y_COMA");
                                            fflush(stdout);
                                            apilarPolaca("DISPLAY");
                                            resetTipos();   }
    ;
...
lectura:
    GET expresion PUNTO_Y_COMA    {   fprintf(stdout,"\nlectura - GET ID PUNTO_Y_COMA"); 
                                fflush(stdout);
                                apilarPolaca("GET");   }
